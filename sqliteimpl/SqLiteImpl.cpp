...
void SqLiteImpl::init(const std::shared_ptr<Region> &region,
  ...
  if (diskProperties != nullptr) {
    auto maxPageCountPtr = diskProperties->find(MAX_PAGE_COUNT_KEY);
    auto pageSizePtr = diskProperties->find(PAGE_SIZE_KEY);
    auto persDir = diskProperties->find(PERSISTENCE_DIR_KEY);
