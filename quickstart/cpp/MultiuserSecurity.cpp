...
void runWithUserRoot(std::shared_ptr<Cache> cachePtr) {
  ...
  std::shared_ptr<CacheableKey> key = CacheableKey::create("Key1");
  ...
  std::shared_ptr<Cacheable> result1Ptr = regionPtr1->get(key);
...
void runWithUserWriter(std::shared_ptr<Cache> cachePtr) {
  ...
  // Get Entries back out of the Region.
  std::shared_ptr<Cacheable> result1Ptr = regionPtr2->get("Key1");
