...
CacheHelper::CacheHelper(const bool isthinClient, const char* poolName,
  ...
  try {
    ...
    thePoolManager()->getFactory()->setPRSingleHopEnabled(prSingleHop);
    thePoolManager()->getFactory()->setThreadLocalConnections(threadLocal);
    ...
    if (locators) {
      ...
      if (serverGroup) {
        thePoolManager()->getFactory()->setServerGroup(serverGroup);
      }
    }
    thePoolManager()->getFactory()->setSubscriptionRedundancy(redundancy);
    thePoolManager()->getFactory()->setSubscriptionEnabled(clientNotification);
    thePoolManager()->getFactory()->setMultiuserAuthentication(isMultiuserMode);
    if (loadConditioningInterval > 0) {
      thePoolManager()->getFactory()->setLoadConditioningInterval(loadConditioningInterval);
    }
    printf("Setting connections to %d ", connections);
    if (connections >= 0) {
      thePoolManager()->getFactory()->setMinConnections(connections);
      thePoolManager()->getFactory()->setMaxConnections(connections);
    }
    if (subscriptionAckInterval != -1) {
      thePoolManager()->getFactory()->setSubscriptionAckInterval(subscriptionAckInterval);
    }
    cachePtr = cacheFac->create();
    thePoolManager()->getFactory()->create(poolName);
...
void CacheHelper::closePool(const char* poolName, bool keepAlive) {
  PoolPtr pool = thePoolManager()->find(poolName);
...
void CacheHelper::addServerLocatorEPs(const char* epList, PoolFactoryPtr pfPtr,
                                      bool poolLocators) {
  LOGERROR("CacheHelper::addServerLocatorEPs1 called");
...
void CacheHelper::addServerLocatorEPs(const char* epList,
                                      ...
                                      bool poolLocators) {
  LOGERROR("CacheHelper::addServerLocatorEPs2 called");
  std::unordered_set<std::string> endpointNames;
  Utils::parseEndpointNamesString(epList, endpointNames);
  for (std::unordered_set<std::string>::iterator iter = endpointNames.begin();
       iter != endpointNames.end(); ++iter) {
    ...
    if (position != std::string::npos) {
      ...
      int portnumber = atoi(((*iter).substr(position + 1)).c_str());
      if (poolLocators) {
        thePoolManager()->getFactory()->addLocator(hostname.c_str(), portnumber);
      } else {
        printf("ankur Server: %d", portnumber);
        thePoolManager()->createFactory()->addServer(hostname.c_str(), portnumber);
      }
    }
  }
}
...
RegionPtr CacheHelper::createPooledRegion(
  ...
  if ((thePoolManager()->find(poolName)) ==
      NULLPTR) {  // Pool does not exist with the same name.

    LOGERROR("******************** Creating Pool %s **********************", poolName);
    PoolPtr pptr = poolFacPtr->create(poolName);
  } else{
    LOGERROR("******************** Found Pool %s **********************", poolName);
  ...
  LOG("******************** Setting Region Factory Options **********************");
  ...
  if (cacheListener != NULLPTR) {
    LOG("******************** Setting CacheListener **********************");

    regionFactoryPtr->setCacheListener(cacheListener);
  }
  LOG("******************** Creating Region **********************");
  RegionPtr rp = regionFactoryPtr->create(name);
  if(rp == NULLPTR)
  {
    LOG("******************** Creating Region FAILED **********************");
  } else
  {
    LOG("******************** Creating Region SUCCESS **********************");
  }
  return rp;
}
...
PoolPtr CacheHelper::getPoolPtr(const char* poolName) {
  return thePoolManager()->find(poolName);
}
