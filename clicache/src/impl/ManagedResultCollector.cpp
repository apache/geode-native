...
namespace apache
{
  namespace geode
  {
    namespace client
    {
      ...
      void ManagedResultCollectorGeneric::addResult(const std::shared_ptr<native::Cacheable>& result)
      {
        try {
     ...
     std::shared_ptr<native::CacheableVector> ManagedResultCollectorGeneric::getResult(System::UInt32 timeout)
      {
        try {
