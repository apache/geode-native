...
class TcrMessageDummy : public TcrMessage {
 public:
  TcrMessageDummy() {}
  explicit TcrMessageDummy(ThinClientBaseDM &tc) { m_tcdm = &tc; }
...
class ThinClientBaseDMDummy : public ThinClientBaseDM {
 public:
  ThinClientBaseDMDummy(TcrConnectionManager &connManager,
      ...
      : ThinClientBaseDM(connManager, theRegion) {}
  ~ThinClientBaseDMDummy() override {}

  GfErrType sendSyncRequest(TcrMessage &, TcrMessageReply &, bool = true,
                            bool = false) override {
    return GF_NOERR;
  }

  GfErrType sendRequestToEP(const TcrMessage &, TcrMessageReply &,
...
TEST(TcrMessage, readVersionTagPart) {
  ...
  auto versionTag =
      message.readVersionTagPart(input, 0, memberListForVersionStamp);
...
TEST(TcrMessage, readCallbackObjectPart2) {
  ...
  const auto &&dataSerializablePrimitive =
      std::dynamic_pointer_cast<DataSerializablePrimitive>(
          message.getCallbackArgumentRef());
  ...
  uint8_t b[] = {3, 'a', 'b', 'c'};
  EXPECT_THAT(b, ::testing::ElementsAreArray(output.getBuffer(),
...
TEST(TcrMessage, readSecureObjectPart2) {
  ...
  const auto &&cacheableString =
      std::dynamic_pointer_cast<CacheableString>(message.getValueRef());
...
TEST(TcrMessage, DISABLED_writeBytesOnly) {
  auto cache = CacheFactory().create();
  ThinClientBaseDMDummy dm(
      CacheRegionHelper::getCacheImpl(&cache)->tcrConnectionManager(), nullptr);
  ...
  message.setData(
      nullptr, 0, 0,
      *CacheRegionHelper::getCacheImpl(&cache)->getSerializationRegistry(),
      memberListForVersionStamp);
  message.writeBytesOnly(cacheableString);
  uint8_t b[] = {
      static_cast<uint8_t>(DSCode::CacheableASCIIString), 0, 3, 'a', 'b', 'c'};
  EXPECT_THAT(b, ::testing::ElementsAreArray(message.getMsgData(),
                                             message.getMsgLength() + 2));
}
