...
template <class T>
using ConnectionQueue = apache::geode::client::FairQueue<T>;
...
TEST(FairQueueTest, putOnClosedDoesNotPutAndObjectClosedAndDestructed) {
  ConnectionQueue<TestObject> queue;
  queue.close();
...
TEST(FairQueueTest, putOnClosedPutsWithOpenQueueTrue) {
  ConnectionQueue<TestObject> queue;
  queue.close();
  queue.put(new TestObject(), true);
  EXPECT_THAT(queue, Not(IsEmpty()));
...
TEST(FairQueueTest, closesAndBecomesEmptyAndObjectClosedAndDestructed) {
  ConnectionQueue<TestObject> queue;
  TestObjectState state;
  queue.put(new TestObject(&state), false);
  queue.close();
...
TEST(FairQueueTest, getNoWaitWith1EntryEmptiesQueue) {
  ConnectionQueue<TestObject> queue;
...
TEST(FairQueueTest, getNoWaitOnEmptyReturnsNullptr) {
  ConnectionQueue<TestObject> queue;
  auto actual = queue.getNoWait();
  EXPECT_THAT(actual, IsNull());
}

TEST(FairQueueTest, getUntilOnEmptyReturnsNullptr) {
  ConnectionQueue<TestObject> queue;
  const auto actual = queue.getUntil(std::chrono::seconds::zero());
  EXPECT_THAT(actual, IsNull());
}

TEST(FairQueueTest, getUntilOnEmptyWaitsAndReturnsNullptr) {
  ConnectionQueue<TestObject> queue;
...
TEST(FairQueueTest, getUntilWith1EntryDoesNotWait) {
  ConnectionQueue<TestObject> queue;
  const auto expected = new TestObject();
  queue.put(expected, false);
  const auto wait = std::chrono::seconds(5);
...
TEST(FairQueueTest, getUntilOnEmptyWaitsAndReturnsObjectPutByAnotherThread) {
...
TEST(FairQueueTest, getUntilOnEmptyWaitsAndReturnsNullWhenClosed) {
  using std::chrono::minutes;
  using std::chrono::seconds;
  using std::chrono::steady_clock;
