...
void putSize(std::shared_ptr<Region>& rptr, const char* buf, int size) {
  char msg[1024];
  std::shared_ptr<CacheableKey> keyPtr = CacheableKey::create(buf);
  uint8_t base = 0;
  ...
  std::shared_ptr<CacheableBytes> valPtr = CacheableBytes::create(valbuf, size);
...
DUNIT_TASK(CLIENT2, VerifyPuts)
  {
    ...
    for (int i = 0; i <= MAX_PAYLOAD; grow(&i)) {
      char keybuf[100];
      sprintf(keybuf, "key%010d", i);
      std::shared_ptr<CacheableKey> keyPtr = CacheableKey::create(keybuf);
