...
int testXmlCacheCreationWithOverflow() {
  ...
  RegionAttributes regAttr2 = regPtr2->getAttributes();
  std::cout << "persistence library = " << regAttr2.getPersistenceLibrary() << std::endl;
  std::cout << "persistence function = " << regAttr2.getPersistenceFactory() << std::endl;
  auto pconfig2 = regAttr2.getPersistenceProperties();
  if (pconfig2 != nullptr) {
