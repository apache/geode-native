...
class OperMonitor : public CacheListener {
  ...
  void check(const EntryEvent& event) {
    ...
    std::shared_ptr<CacheableKey> key = event.getKey();
...
void initClientCache(int durableIdx, int redundancy, int durableTimeout,
  ...
  std::shared_ptr<CacheableKey> ldkey = CacheableKey::create(mixKeys[3]);
  v.push_back(ldkey);
  regPtr0->registerKeys(v, true);
  regPtr1->registerKeys(v, true);
  v.clear();
  std::shared_ptr<CacheableKey> lkey = CacheableKey::create(mixKeys[2]);
  v.push_back(lkey);
